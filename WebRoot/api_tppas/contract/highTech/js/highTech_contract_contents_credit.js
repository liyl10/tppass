...
$(function() {
	$(document).ready(function() {
		...
		// 设定金额的初期值，如果为空的话设置为0
		// 第三条
		defaultDataSet("partySubsidizePartyb");
		defaultDataSet("partybNewInvestments");
		defaultDataSet("partycSubsidizePartyb");
		
		// 第四条
		var idList = $("input[id^='fundsPlanYuan']");
		// 添加输入项信息
		for ( var i = 0; i < idList.length; i++) {
			var id = idList.eq(i).attr("id");
			defaultDataSet(id);
		}
		/*defaultDataSet("fundsPlanYuan1");
		defaultDataSet("fundsPlanYuan2");
		defaultDataSet("fundsPlanYuan3");*/
		
		// 第四条
		defaultDataSet("fundingSchemesYuan");
		defaultDataSet("fundingSchemesQualified");
		defaultDataSet("fundingSchemesBasic");
		
		// 第七条
		defaultDataSet("totalInvestmentNewXm");
		defaultDataSet("cumulativeSalesOverallXm");
		defaultDataSet("accumulatedTaxOverallXm");
		defaultDataSet("accumulatedProfitsOverallXm");
		defaultDataSet("assetSizeOverallXm");
		defaultDataSet("employmentNewOverallXm");
		
		defaultDataSet("cumulativeSalesEconomy");
		defaultDataSet("accumulatedTaxEconomy");
		defaultDataSet("accumulatedProfitsEconomy");
...
$("#nextBtn").bind("click", function() {
	...
	var actionUrl= path + "/api/contract/tcontractContentsA!ContentsTofundUseCredit.action?now="+ new Date().getTime();
		upms.showOverLay();// 打开遮罩
		var $newPgDiv = $("#content");
		var arrParams = [{name:"tcontractId",targetid:"tcontractId",type:"text"},
		                 {name:"tcontractContentsA.implementationPlan",targetid:"implementationPlan",type:"text"},
