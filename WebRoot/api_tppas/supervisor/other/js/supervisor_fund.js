	...
	$("#saveBut").bind("click", function(){
		...
		//判断是保存还是修改
		if($("#fundBId").val()!=null&&$("#fundBId").val()!=''){
		...
		if (confirm(comfirmInfo)){
			...
			var actionUrl= path + "/api/supervisor/supervisorFundBAction!saveOrNextFundB.action?optType=0&now=" + new Date().getTime();
			upms.showOverLay();// 打开遮罩
			var $newPgDiv = $("#content");
			var arrParams = [{name:"tsupervisorFundB.commentInfo",targetid:"commentInfo",type:"text"}];
	...
	$("#nextBut").bind("click", function(){
		...
		if (confirm(comfirmInfo)){
			...
			var actionUrl= path + "/api/supervisor/supervisorFundBAction!saveOrNextFundB.action?optType=1&now=" + new Date().getTime();
			upms.showOverLay();// 打开遮罩
			var $newPgDiv = $("#content");
			var arrParams = [{name:"tsupervisorFundB.commentInfo",targetid:"commentInfo",type:"text"}];
	...
	$("#nextStep").bind("click", function(){
		//选中左边菜单
		upms.upmsUtils.setMenu();
		var actionUrl= path + "/api/supervisor/projectCompleteInfoAction!getProjectCompleteInfo.action?supervisorId="+$("#supervisorId").val()+"&isEdit="+$("#isEdit").val()+"&now=" + new Date().getTime();
		upms.showOverLay();// 打开遮罩
		var $newPgDiv = $("#content");
		var arrParams = [];
		var data = upms.transParsToObj(arrParams, $newPgDiv);
