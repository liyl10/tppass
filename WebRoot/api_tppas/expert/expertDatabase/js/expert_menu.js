	...
	$(function() {
	    $(document).ready(function () {
			...
			$("#content").css("height", value);
 			...
 			$("#content").load(path + "/api/expert/texpertAction!getExpertInfo.action?expertId="+$("#expertId").val()+"&isEdit="+$("#isEdit").val()+"&now="+new Date().getTime());
		});
	});


	//基本信息
	function showInfo(expertId,isEdit){
		var actionUrl= path + "/api/expert/texpertAction!getExpertInfo.action?expertId="+expertId+"&isEdit="+isEdit+"&now="+new Date().getTime();
		rect(actionUrl);
	}
	
	//技术研究成果
	function showJsyjcg(expertId,isEdit){
		var actionUrl= path + "/api/expert/ttechnologyGainAction!list.action?expertId="+expertId+"&isEdit="+isEdit+"&now="+new Date().getTime();
		rect(actionUrl);
	 }
