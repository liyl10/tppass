...
<script type="text/javascript" src="${pageContext.request.contextPath}/api_tppas/super/js/acceptanceModel_list.js"></script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	class="t-list">
	<tr>
		<th style="text-align:center;white-space:nowrap">验收模板名称</th>
	...
	<s:if test="pager.list.size  > 0">
		<s:iterator value="pager.list" status="rowData">
			<tr>
				<td><s:property value='%{acceptanceTemplateName}' escape='true' /></td>
...
<%-- 				<tag:auth code="ss.acceptanceModel_list.update"> --%>
				<a href="javascript: void(0);" name="detailBtn"
					onclick="doUpdate(
					'<s:property value='%{acceptanceTemplateId}' escape='true'/>');">修改</a>
<%-- 					</tag:auth> --%>
<%-- 					<tag:auth code="ss.acceptanceModel_list.delete"> --%>
					<a href="javascript: void(0);" name="delBtn"
					onclick="doDel('<s:property value='%{acceptanceTemplateId}' escape='true'/>');">删除</a>
<%-- 					</tag:auth> --%>
					</td>
			</tr>
		</s:iterator>
