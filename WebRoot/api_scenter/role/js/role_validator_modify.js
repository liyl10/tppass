...
$(document).ready(function(){
	 ...
	 * 表单验证
	 */
	var validform=$("#role_modify_form").Validform({
		tiptype:3,
		showAllError:true,
		datatype:{
	...
	$("input[name='role_name']").focusout(function(){
		validform.addRule([{
			...
			datatype:"roleName",
			ajaxurl:url_cur+"/api/role/role_ajaxCheckName.action?role_name="+$('#role_name').val()+"&role_id="+$('#role_id').val(),
	...
	$("[datatype]").focusin(function(){
		...
		//$(this).attr("value",trim_yxy($(this).val()));   //去除首尾空格
	});		 	 
		   //jquery end						   
 });