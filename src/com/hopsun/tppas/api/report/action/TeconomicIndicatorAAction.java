...
/**
 * @comment 执行期内企业完成的经济指标
 * @author liush
 * @DATE: 2013-8-23 @TIME: 下午3:16:53
 * @Vsersion: 1.0
 */
public class TeconomicIndicatorAAction extends BaseAction {
	public final static Logger LOGGER = Logger.getLogger(TeconomicIndicatorAAction.class.getName());

	private static final long serialVersionUID = 1621625277256992400L;
	@Resource
	private TeconomicIndicatorAService teconomicIndicatorAService;
	@Resource
	private TprojectApplicationService tprojectApplicationService;
	/** 项目申报信息 */
	private TprojectApplication tprojectApplication;
	private TeconomicIndicatorA teconomicIndicator;
	/** 项目id */
	...
	/** 新增就业人数累计 */
	private double newEmploymentList1;
	...
	private BigDecimal newEmploymentList;
	...
	public String initHighTech() {
		...
		this.nowYear = Integer.valueOf(projectTime);
		this.teconomicIndicator = teconomicIndicatorAService.get("tprojectApplication.projectId", projectId);
		...
		if (teconomicIndicator != null) {
			/** 新增投资累计 */
			outputValueList1 = teconomicIndicator.getNewInvest2() + teconomicIndicator.getNewInvest1() + teconomicIndicator.getNewInvest();
			...
			/** 资产规模累计 */
			earnMoneyList1 = teconomicIndicator.getAssetSize2() + teconomicIndicator.getAssetSize1() + teconomicIndicator.getAssetSize();
			earnMoneyList = new BigDecimal(earnMoneyList1);
			earnMoneyList = earnMoneyList.setScale(4, BigDecimal.ROUND_HALF_UP);

			/** 新增就业人数累计 */
			newEmploymentList1 = teconomicIndicator.getNewEmployment2() + teconomicIndicator.getNewEmployment1() + teconomicIndicator.getNewEmployment2();
			newEmploymentList = new BigDecimal(newEmploymentList1);
			newEmploymentList = newEmploymentList.setScale(4, BigDecimal.ROUND_HALF_UP);
		}
		return "initHighTech";
	}
	...
	public String initOther() {
		...
		HttpSession session = this.getRequest().getSession();
		ScUsers user = (ScUsers) session.getAttribute("sysUser");
		// session.setAttribute(Constants.SESSION_CMDKEY, "companyQuotaOther");
	...
	/**
	 * @return the teconomicIndicatorAService
	 */
	public TeconomicIndicatorAService getTeconomicIndicatorAService() {
		return teconomicIndicatorAService;
	}

	/**
	 * @param teconomicIndicatorAService
	 *            the teconomicIndicatorAService to set
	 */
	public void setTeconomicIndicatorAService(TeconomicIndicatorAService teconomicIndicatorAService) {
		this.teconomicIndicatorAService = teconomicIndicatorAService;
	}
	 ...
	 */
	public TeconomicIndicatorA getTeconomicIndicator() {
	 ...
	 */
	public void setTeconomicIndicator(TeconomicIndicatorA teconomicIndicator) {
